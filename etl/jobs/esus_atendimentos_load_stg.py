        ...
        for raw in reader:
            rows.append(
                (
                    ...
                    _parse_date(raw.get("data_atendimento") or ""),
                    raw.get("codigo_unidade") or None,
                    raw.get("codigo_equipe") or None,
                    raw.get("cbo_profissional") or None,
                    raw.get("hash_identificador_paciente") or None,
                    raw.get("tipo_atendimento") or None,
                    raw.get("codigo_proced") or None,
                    float(raw.get("quantidade") or 0) if raw.get("quantidade") else 1,
...
def load_atendimentos_to_stg(csv_path: Path) -> int:
    ...
    cur.executemany(
        """
        INSERT INTO stg.stg_esus_atendimentos (
            codigo_ibge_municipio, data_atendimento, codigo_unidade, codigo_equipe,
            cbo_profissional, hash_identificador_paciente, tipo_atendimento,
            codigo_proced, quantidade, codigo_territorio
        )
